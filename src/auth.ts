...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      if (account?.provider === "google") {
        ...
        try {
          ...
          //Buscada à parte de "existingUser": create()/update() abaixo não devolvem
          //authProviders, então só essa variável carrega esse campo.
          const usuarioComProviders = await db.user.findUnique({
          ...
          if (!usuarioComProviders) {
            ...
            existingUser = usuarioComProviders;
    ...
    async jwt(params) {
      ...
      //por isso não pode viver em auth.config.ts (Edge Runtime não roda Prisma)
      if (trigger === "update") {
