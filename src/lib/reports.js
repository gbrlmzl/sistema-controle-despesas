...
import { competenciaSeguinte } from "./expenses";
...
function filtroBase(residenceId, month, year, userId) {
    return {
        residenceId: residenceId,
        month: month,
        year: year,
        deletedAt: null, //RN-057: lançamentos excluídos ficam de fora do relatório
        ...(userId ? { createdById: userId } : {}),
    };
}

export function competenciaAnterior({ month, year }) {
...
export async function relatorioPorCategoria(residenceId, month, year, userId = null) {
...
export async function compararCompetencias(residenceId, atual, anterior, userId = null) {
...
export async function serieDeEvolucao(residenceId, ate, quantidade = COMPETENCIAS_NA_EVOLUCAO, userId = null) {
    //Monta a janela andando para trás a partir da competência exibida
    const janela = [];
    let cursor = { month: ate.month, year: ate.year };
...
export async function calcularRateio(residenceId, month, year) {
...
export async function mediaPorCategoria(residenceId, ate, userId = null) {
    const janela = [];
...
export async function relatorioComDesvios(residenceId, competencia, userId = null) {
    ...
    const categorias = relatorio.categorias.map(categoria => {
        ...
        if (!media || !media.confiavel || media.mediaInCents === 0) {
            return { ...categoria, mediaInCents: media?.mediaInCents ?? null, desvio: null, acimaDaMedia: null };
        }
...
export async function despesasParaExportacao(residenceId, month, year, userId = null) {
